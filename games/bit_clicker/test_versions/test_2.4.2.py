...
def load_game(show_popup=True):  # ✅ Added optional parameter
    global game_data
    global game_save
    ...
    try:
        ...
        # Start bps thread only if not already running
        if game_data.get("bps_running") and not threading.active_count() > 1:  
            game_data["bps_running"] = True
            threading.Thread(target=bits_per_second, daemon=True).start()

        if show_popup:  # ✅ Only show pop-up when `show_popup=True`
            messagebox.showinfo("Load Game", "Game loaded successfully!")

        # Save updated file in case new defaults were added
        save_game(show_popup=False)  # ✅ Save silently after loading

    except FileNotFoundError:
        messagebox.showwarning("Load Game", "No save file found, starting a new game.")
        game_data = DEFAULT_GAME_DATA.copy()
        save_game(show_popup=False)  # ✅ Save silently after creating a new game
...
def bits_per_second():
    while game_data.get("bps_running"):  # Only run while the flag is True
        sleep(1)
        game_data['bits'] += game_data['bits_second']
        
        # Use root.after to safely update the GUI from the main thread
        root.after(0, update_label)  # Schedule update_label() to be run in the main thread
...
filemenu.add_command(label="Save Game", command=lambda: save_game(show_popup=True))
filemenu.add_command(label="Load Game", command=lambda: load_game(show_popup=True))
filemenu.add_separator()
filemenu.add_command(label="Save and Quit", command=lambda: save_game(quit_after_save=True, show_popup=False))  # ✅ No pop-up
